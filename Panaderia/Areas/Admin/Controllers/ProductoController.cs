...
namespace Panaderia.Areas.Admin.Controllers
{
    ...
    public class ProductoController : Controller
    {
        ...
        public async Task<IActionResult> Upsert(ProductoVM productoVM)
        {
            if(ModelState.IsValid)
            {
                ...
                if(productoVM.Producto.Id == 0)
                {
                    ...
                    productoVM.Producto.ImagegUrl = fileName + extension;
                ...
                else
                {
                    ...
                    if(files.Count > 0) //Si se carga una imagen nueva
                    {
                        string upload = webRootPath + DS.ImagenRuta;
                        string fileName = Guid.NewGuid().ToString();
                        string extension = Path.GetExtension(files[0].FileName);
                        ...
                        var anteriorFile = Path.Combine(upload, objProducto.ImagegUrl);
                        ...
                        productoVM.Producto.ImagegUrl = fileName + extension;
                    } // En caso no se carga una nueva imagen
                    else
                    {
                        productoVM.Producto.ImagegUrl = objProducto.ImagegUrl;
                    }
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            var anteriorFile = Path.Combine(upload, productoDB.ImagegUrl);
            if(System.IO.File.Exists(anteriorFile))
            {
                System.IO.File.Delete(anteriorFile);
            }
